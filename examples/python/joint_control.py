...
class ArmJointControl(Node):
    ...
    def move_arm_joints_toppra(self, target_positions: list, v_max=0.8, a_max=2.5, 
                                dt=0.005, wait_for_completion=True):

        if not self.controller_switch.is_controller_active("joint_position_controller"):
            self.get_logger().error("joint position control mode is not active")
            return False
        time.sleep(1)
...
def main(args=None):
    ...
    try:
        ...
        if not controller_switch.is_controller_active("joint_position_controller"):
            print("joint position control mode is not active")
            if not controller_switch.switch_to_position_control():
