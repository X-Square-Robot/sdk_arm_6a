...
class ControllerSwitcherService:
    ...
    def is_controller_active(self, controller_name: str):
        """check if controller is active"""
        return self.list_service.get_controller_state(controller_name) == "active"
    ...
    def safe_switch(self, activate_list=None, deactivate_list=None):
        ...
        for controller in activate_list:
            if not self.is_controller_active(controller):
                self.node.get_logger().error(f"Controller '{controller}' not activated")
                all_success = False
