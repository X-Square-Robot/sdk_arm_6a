...
class ArmJointControl(Node):
    ...
    def move_arm_endpose_toppra(self, target_pose , v_max=1.5, a_max=2.0):
    ...
    def move_to_zero_pose(self):
        target_pose = Pose()
        target_pose.position = Point(x=0.0, y=0.0, z=0.001)
...
def main(args=None):
    ...
    try:
        ...
        time.sleep(1)
        target = Pose()
        target.position = Point(x=0.3511, y=0.0, z=0.2110)
        target.orientation = Quaternion(x=0.0047, y=-0.0670, z=-0.0000, w=0.9977)
