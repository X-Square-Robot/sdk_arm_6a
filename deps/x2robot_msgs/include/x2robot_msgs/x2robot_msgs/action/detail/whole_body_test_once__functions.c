// generated from rosidl_generator_c/resource/idl__functions.c.em
// with input from x2robot_msgs:action/WholeBodyTestOnce.idl
// generated code does not contain a copyright notice
#include "x2robot_msgs/action/detail/whole_body_test_once__functions.h"

#include <assert.h>
#include <stdbool.h>
#include <stdlib.h>
#include <string.h>

#include "rcutils/allocator.h"


bool
x2robot_msgs__action__WholeBodyTestOnce_Goal__init(x2robot_msgs__action__WholeBodyTestOnce_Goal * msg)
{
  if (!msg) {
    return false;
  }
  // max_velocity_scaling_factor
  msg->max_velocity_scaling_factor = 0.1f;
  // max_acceleration_scaling_factor
  msg->max_acceleration_scaling_factor = 0.2f;
  return true;
}

void
x2robot_msgs__action__WholeBodyTestOnce_Goal__fini(x2robot_msgs__action__WholeBodyTestOnce_Goal * msg)
{
  if (!msg) {
    return;
  }
  // max_velocity_scaling_factor
  // max_acceleration_scaling_factor
}

bool
x2robot_msgs__action__WholeBodyTestOnce_Goal__are_equal(const x2robot_msgs__action__WholeBodyTestOnce_Goal * lhs, const x2robot_msgs__action__WholeBodyTestOnce_Goal * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // max_velocity_scaling_factor
  if (lhs->max_velocity_scaling_factor != rhs->max_velocity_scaling_factor) {
    return false;
  }
  // max_acceleration_scaling_factor
  if (lhs->max_acceleration_scaling_factor != rhs->max_acceleration_scaling_factor) {
    return false;
  }
  return true;
}

bool
x2robot_msgs__action__WholeBodyTestOnce_Goal__copy(
  const x2robot_msgs__action__WholeBodyTestOnce_Goal * input,
  x2robot_msgs__action__WholeBodyTestOnce_Goal * output)
{
  if (!input || !output) {
    return false;
  }
  // max_velocity_scaling_factor
  output->max_velocity_scaling_factor = input->max_velocity_scaling_factor;
  // max_acceleration_scaling_factor
  output->max_acceleration_scaling_factor = input->max_acceleration_scaling_factor;
  return true;
}

x2robot_msgs__action__WholeBodyTestOnce_Goal *
x2robot_msgs__action__WholeBodyTestOnce_Goal__create(void)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  x2robot_msgs__action__WholeBodyTestOnce_Goal * msg = (x2robot_msgs__action__WholeBodyTestOnce_Goal *)allocator.allocate(sizeof(x2robot_msgs__action__WholeBodyTestOnce_Goal), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(x2robot_msgs__action__WholeBodyTestOnce_Goal));
  bool success = x2robot_msgs__action__WholeBodyTestOnce_Goal__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
x2robot_msgs__action__WholeBodyTestOnce_Goal__destroy(x2robot_msgs__action__WholeBodyTestOnce_Goal * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    x2robot_msgs__action__WholeBodyTestOnce_Goal__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
x2robot_msgs__action__WholeBodyTestOnce_Goal__Sequence__init(x2robot_msgs__action__WholeBodyTestOnce_Goal__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  x2robot_msgs__action__WholeBodyTestOnce_Goal * data = NULL;

  if (size) {
    data = (x2robot_msgs__action__WholeBodyTestOnce_Goal *)allocator.zero_allocate(size, sizeof(x2robot_msgs__action__WholeBodyTestOnce_Goal), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = x2robot_msgs__action__WholeBodyTestOnce_Goal__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        x2robot_msgs__action__WholeBodyTestOnce_Goal__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
x2robot_msgs__action__WholeBodyTestOnce_Goal__Sequence__fini(x2robot_msgs__action__WholeBodyTestOnce_Goal__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      x2robot_msgs__action__WholeBodyTestOnce_Goal__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

x2robot_msgs__action__WholeBodyTestOnce_Goal__Sequence *
x2robot_msgs__action__WholeBodyTestOnce_Goal__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  x2robot_msgs__action__WholeBodyTestOnce_Goal__Sequence * array = (x2robot_msgs__action__WholeBodyTestOnce_Goal__Sequence *)allocator.allocate(sizeof(x2robot_msgs__action__WholeBodyTestOnce_Goal__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = x2robot_msgs__action__WholeBodyTestOnce_Goal__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
x2robot_msgs__action__WholeBodyTestOnce_Goal__Sequence__destroy(x2robot_msgs__action__WholeBodyTestOnce_Goal__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    x2robot_msgs__action__WholeBodyTestOnce_Goal__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
x2robot_msgs__action__WholeBodyTestOnce_Goal__Sequence__are_equal(const x2robot_msgs__action__WholeBodyTestOnce_Goal__Sequence * lhs, const x2robot_msgs__action__WholeBodyTestOnce_Goal__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!x2robot_msgs__action__WholeBodyTestOnce_Goal__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
x2robot_msgs__action__WholeBodyTestOnce_Goal__Sequence__copy(
  const x2robot_msgs__action__WholeBodyTestOnce_Goal__Sequence * input,
  x2robot_msgs__action__WholeBodyTestOnce_Goal__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(x2robot_msgs__action__WholeBodyTestOnce_Goal);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    x2robot_msgs__action__WholeBodyTestOnce_Goal * data =
      (x2robot_msgs__action__WholeBodyTestOnce_Goal *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!x2robot_msgs__action__WholeBodyTestOnce_Goal__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          x2robot_msgs__action__WholeBodyTestOnce_Goal__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!x2robot_msgs__action__WholeBodyTestOnce_Goal__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `message`
// Member `successful_poses`
// Member `failed_poses`
#include "rosidl_runtime_c/string_functions.h"

bool
x2robot_msgs__action__WholeBodyTestOnce_Result__init(x2robot_msgs__action__WholeBodyTestOnce_Result * msg)
{
  if (!msg) {
    return false;
  }
  // success
  // message
  if (!rosidl_runtime_c__String__init(&msg->message)) {
    x2robot_msgs__action__WholeBodyTestOnce_Result__fini(msg);
    return false;
  }
  // successful_poses
  if (!rosidl_runtime_c__String__Sequence__init(&msg->successful_poses, 0)) {
    x2robot_msgs__action__WholeBodyTestOnce_Result__fini(msg);
    return false;
  }
  // failed_poses
  if (!rosidl_runtime_c__String__Sequence__init(&msg->failed_poses, 0)) {
    x2robot_msgs__action__WholeBodyTestOnce_Result__fini(msg);
    return false;
  }
  return true;
}

void
x2robot_msgs__action__WholeBodyTestOnce_Result__fini(x2robot_msgs__action__WholeBodyTestOnce_Result * msg)
{
  if (!msg) {
    return;
  }
  // success
  // message
  rosidl_runtime_c__String__fini(&msg->message);
  // successful_poses
  rosidl_runtime_c__String__Sequence__fini(&msg->successful_poses);
  // failed_poses
  rosidl_runtime_c__String__Sequence__fini(&msg->failed_poses);
}

bool
x2robot_msgs__action__WholeBodyTestOnce_Result__are_equal(const x2robot_msgs__action__WholeBodyTestOnce_Result * lhs, const x2robot_msgs__action__WholeBodyTestOnce_Result * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // success
  if (lhs->success != rhs->success) {
    return false;
  }
  // message
  if (!rosidl_runtime_c__String__are_equal(
      &(lhs->message), &(rhs->message)))
  {
    return false;
  }
  // successful_poses
  if (!rosidl_runtime_c__String__Sequence__are_equal(
      &(lhs->successful_poses), &(rhs->successful_poses)))
  {
    return false;
  }
  // failed_poses
  if (!rosidl_runtime_c__String__Sequence__are_equal(
      &(lhs->failed_poses), &(rhs->failed_poses)))
  {
    return false;
  }
  return true;
}

bool
x2robot_msgs__action__WholeBodyTestOnce_Result__copy(
  const x2robot_msgs__action__WholeBodyTestOnce_Result * input,
  x2robot_msgs__action__WholeBodyTestOnce_Result * output)
{
  if (!input || !output) {
    return false;
  }
  // success
  output->success = input->success;
  // message
  if (!rosidl_runtime_c__String__copy(
      &(input->message), &(output->message)))
  {
    return false;
  }
  // successful_poses
  if (!rosidl_runtime_c__String__Sequence__copy(
      &(input->successful_poses), &(output->successful_poses)))
  {
    return false;
  }
  // failed_poses
  if (!rosidl_runtime_c__String__Sequence__copy(
      &(input->failed_poses), &(output->failed_poses)))
  {
    return false;
  }
  return true;
}

x2robot_msgs__action__WholeBodyTestOnce_Result *
x2robot_msgs__action__WholeBodyTestOnce_Result__create(void)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  x2robot_msgs__action__WholeBodyTestOnce_Result * msg = (x2robot_msgs__action__WholeBodyTestOnce_Result *)allocator.allocate(sizeof(x2robot_msgs__action__WholeBodyTestOnce_Result), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(x2robot_msgs__action__WholeBodyTestOnce_Result));
  bool success = x2robot_msgs__action__WholeBodyTestOnce_Result__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
x2robot_msgs__action__WholeBodyTestOnce_Result__destroy(x2robot_msgs__action__WholeBodyTestOnce_Result * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    x2robot_msgs__action__WholeBodyTestOnce_Result__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
x2robot_msgs__action__WholeBodyTestOnce_Result__Sequence__init(x2robot_msgs__action__WholeBodyTestOnce_Result__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  x2robot_msgs__action__WholeBodyTestOnce_Result * data = NULL;

  if (size) {
    data = (x2robot_msgs__action__WholeBodyTestOnce_Result *)allocator.zero_allocate(size, sizeof(x2robot_msgs__action__WholeBodyTestOnce_Result), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = x2robot_msgs__action__WholeBodyTestOnce_Result__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        x2robot_msgs__action__WholeBodyTestOnce_Result__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
x2robot_msgs__action__WholeBodyTestOnce_Result__Sequence__fini(x2robot_msgs__action__WholeBodyTestOnce_Result__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      x2robot_msgs__action__WholeBodyTestOnce_Result__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

x2robot_msgs__action__WholeBodyTestOnce_Result__Sequence *
x2robot_msgs__action__WholeBodyTestOnce_Result__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  x2robot_msgs__action__WholeBodyTestOnce_Result__Sequence * array = (x2robot_msgs__action__WholeBodyTestOnce_Result__Sequence *)allocator.allocate(sizeof(x2robot_msgs__action__WholeBodyTestOnce_Result__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = x2robot_msgs__action__WholeBodyTestOnce_Result__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
x2robot_msgs__action__WholeBodyTestOnce_Result__Sequence__destroy(x2robot_msgs__action__WholeBodyTestOnce_Result__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    x2robot_msgs__action__WholeBodyTestOnce_Result__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
x2robot_msgs__action__WholeBodyTestOnce_Result__Sequence__are_equal(const x2robot_msgs__action__WholeBodyTestOnce_Result__Sequence * lhs, const x2robot_msgs__action__WholeBodyTestOnce_Result__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!x2robot_msgs__action__WholeBodyTestOnce_Result__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
x2robot_msgs__action__WholeBodyTestOnce_Result__Sequence__copy(
  const x2robot_msgs__action__WholeBodyTestOnce_Result__Sequence * input,
  x2robot_msgs__action__WholeBodyTestOnce_Result__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(x2robot_msgs__action__WholeBodyTestOnce_Result);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    x2robot_msgs__action__WholeBodyTestOnce_Result * data =
      (x2robot_msgs__action__WholeBodyTestOnce_Result *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!x2robot_msgs__action__WholeBodyTestOnce_Result__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          x2robot_msgs__action__WholeBodyTestOnce_Result__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!x2robot_msgs__action__WholeBodyTestOnce_Result__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `current_pose`
// Member `successful_poses`
// Member `failed_poses`
// already included above
// #include "rosidl_runtime_c/string_functions.h"

bool
x2robot_msgs__action__WholeBodyTestOnce_Feedback__init(x2robot_msgs__action__WholeBodyTestOnce_Feedback * msg)
{
  if (!msg) {
    return false;
  }
  // current_pose
  if (!rosidl_runtime_c__String__init(&msg->current_pose)) {
    x2robot_msgs__action__WholeBodyTestOnce_Feedback__fini(msg);
    return false;
  }
  // progress
  // successful_poses
  if (!rosidl_runtime_c__String__Sequence__init(&msg->successful_poses, 0)) {
    x2robot_msgs__action__WholeBodyTestOnce_Feedback__fini(msg);
    return false;
  }
  // failed_poses
  if (!rosidl_runtime_c__String__Sequence__init(&msg->failed_poses, 0)) {
    x2robot_msgs__action__WholeBodyTestOnce_Feedback__fini(msg);
    return false;
  }
  // total_poses
  // current_index
  return true;
}

void
x2robot_msgs__action__WholeBodyTestOnce_Feedback__fini(x2robot_msgs__action__WholeBodyTestOnce_Feedback * msg)
{
  if (!msg) {
    return;
  }
  // current_pose
  rosidl_runtime_c__String__fini(&msg->current_pose);
  // progress
  // successful_poses
  rosidl_runtime_c__String__Sequence__fini(&msg->successful_poses);
  // failed_poses
  rosidl_runtime_c__String__Sequence__fini(&msg->failed_poses);
  // total_poses
  // current_index
}

bool
x2robot_msgs__action__WholeBodyTestOnce_Feedback__are_equal(const x2robot_msgs__action__WholeBodyTestOnce_Feedback * lhs, const x2robot_msgs__action__WholeBodyTestOnce_Feedback * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // current_pose
  if (!rosidl_runtime_c__String__are_equal(
      &(lhs->current_pose), &(rhs->current_pose)))
  {
    return false;
  }
  // progress
  if (lhs->progress != rhs->progress) {
    return false;
  }
  // successful_poses
  if (!rosidl_runtime_c__String__Sequence__are_equal(
      &(lhs->successful_poses), &(rhs->successful_poses)))
  {
    return false;
  }
  // failed_poses
  if (!rosidl_runtime_c__String__Sequence__are_equal(
      &(lhs->failed_poses), &(rhs->failed_poses)))
  {
    return false;
  }
  // total_poses
  if (lhs->total_poses != rhs->total_poses) {
    return false;
  }
  // current_index
  if (lhs->current_index != rhs->current_index) {
    return false;
  }
  return true;
}

bool
x2robot_msgs__action__WholeBodyTestOnce_Feedback__copy(
  const x2robot_msgs__action__WholeBodyTestOnce_Feedback * input,
  x2robot_msgs__action__WholeBodyTestOnce_Feedback * output)
{
  if (!input || !output) {
    return false;
  }
  // current_pose
  if (!rosidl_runtime_c__String__copy(
      &(input->current_pose), &(output->current_pose)))
  {
    return false;
  }
  // progress
  output->progress = input->progress;
  // successful_poses
  if (!rosidl_runtime_c__String__Sequence__copy(
      &(input->successful_poses), &(output->successful_poses)))
  {
    return false;
  }
  // failed_poses
  if (!rosidl_runtime_c__String__Sequence__copy(
      &(input->failed_poses), &(output->failed_poses)))
  {
    return false;
  }
  // total_poses
  output->total_poses = input->total_poses;
  // current_index
  output->current_index = input->current_index;
  return true;
}

x2robot_msgs__action__WholeBodyTestOnce_Feedback *
x2robot_msgs__action__WholeBodyTestOnce_Feedback__create(void)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  x2robot_msgs__action__WholeBodyTestOnce_Feedback * msg = (x2robot_msgs__action__WholeBodyTestOnce_Feedback *)allocator.allocate(sizeof(x2robot_msgs__action__WholeBodyTestOnce_Feedback), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(x2robot_msgs__action__WholeBodyTestOnce_Feedback));
  bool success = x2robot_msgs__action__WholeBodyTestOnce_Feedback__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
x2robot_msgs__action__WholeBodyTestOnce_Feedback__destroy(x2robot_msgs__action__WholeBodyTestOnce_Feedback * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    x2robot_msgs__action__WholeBodyTestOnce_Feedback__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
x2robot_msgs__action__WholeBodyTestOnce_Feedback__Sequence__init(x2robot_msgs__action__WholeBodyTestOnce_Feedback__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  x2robot_msgs__action__WholeBodyTestOnce_Feedback * data = NULL;

  if (size) {
    data = (x2robot_msgs__action__WholeBodyTestOnce_Feedback *)allocator.zero_allocate(size, sizeof(x2robot_msgs__action__WholeBodyTestOnce_Feedback), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = x2robot_msgs__action__WholeBodyTestOnce_Feedback__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        x2robot_msgs__action__WholeBodyTestOnce_Feedback__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
x2robot_msgs__action__WholeBodyTestOnce_Feedback__Sequence__fini(x2robot_msgs__action__WholeBodyTestOnce_Feedback__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      x2robot_msgs__action__WholeBodyTestOnce_Feedback__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

x2robot_msgs__action__WholeBodyTestOnce_Feedback__Sequence *
x2robot_msgs__action__WholeBodyTestOnce_Feedback__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  x2robot_msgs__action__WholeBodyTestOnce_Feedback__Sequence * array = (x2robot_msgs__action__WholeBodyTestOnce_Feedback__Sequence *)allocator.allocate(sizeof(x2robot_msgs__action__WholeBodyTestOnce_Feedback__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = x2robot_msgs__action__WholeBodyTestOnce_Feedback__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
x2robot_msgs__action__WholeBodyTestOnce_Feedback__Sequence__destroy(x2robot_msgs__action__WholeBodyTestOnce_Feedback__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    x2robot_msgs__action__WholeBodyTestOnce_Feedback__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
x2robot_msgs__action__WholeBodyTestOnce_Feedback__Sequence__are_equal(const x2robot_msgs__action__WholeBodyTestOnce_Feedback__Sequence * lhs, const x2robot_msgs__action__WholeBodyTestOnce_Feedback__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!x2robot_msgs__action__WholeBodyTestOnce_Feedback__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
x2robot_msgs__action__WholeBodyTestOnce_Feedback__Sequence__copy(
  const x2robot_msgs__action__WholeBodyTestOnce_Feedback__Sequence * input,
  x2robot_msgs__action__WholeBodyTestOnce_Feedback__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(x2robot_msgs__action__WholeBodyTestOnce_Feedback);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    x2robot_msgs__action__WholeBodyTestOnce_Feedback * data =
      (x2robot_msgs__action__WholeBodyTestOnce_Feedback *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!x2robot_msgs__action__WholeBodyTestOnce_Feedback__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          x2robot_msgs__action__WholeBodyTestOnce_Feedback__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!x2robot_msgs__action__WholeBodyTestOnce_Feedback__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `goal_id`
#include "unique_identifier_msgs/msg/detail/uuid__functions.h"
// Member `goal`
// already included above
// #include "x2robot_msgs/action/detail/whole_body_test_once__functions.h"

bool
x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Request__init(x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Request * msg)
{
  if (!msg) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__init(&msg->goal_id)) {
    x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Request__fini(msg);
    return false;
  }
  // goal
  if (!x2robot_msgs__action__WholeBodyTestOnce_Goal__init(&msg->goal)) {
    x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Request__fini(msg);
    return false;
  }
  return true;
}

void
x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Request__fini(x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Request * msg)
{
  if (!msg) {
    return;
  }
  // goal_id
  unique_identifier_msgs__msg__UUID__fini(&msg->goal_id);
  // goal
  x2robot_msgs__action__WholeBodyTestOnce_Goal__fini(&msg->goal);
}

bool
x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Request__are_equal(const x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Request * lhs, const x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Request * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__are_equal(
      &(lhs->goal_id), &(rhs->goal_id)))
  {
    return false;
  }
  // goal
  if (!x2robot_msgs__action__WholeBodyTestOnce_Goal__are_equal(
      &(lhs->goal), &(rhs->goal)))
  {
    return false;
  }
  return true;
}

bool
x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Request__copy(
  const x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Request * input,
  x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Request * output)
{
  if (!input || !output) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__copy(
      &(input->goal_id), &(output->goal_id)))
  {
    return false;
  }
  // goal
  if (!x2robot_msgs__action__WholeBodyTestOnce_Goal__copy(
      &(input->goal), &(output->goal)))
  {
    return false;
  }
  return true;
}

x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Request *
x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Request__create(void)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Request * msg = (x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Request *)allocator.allocate(sizeof(x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Request), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Request));
  bool success = x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Request__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Request__destroy(x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Request * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Request__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Request__Sequence__init(x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Request__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Request * data = NULL;

  if (size) {
    data = (x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Request *)allocator.zero_allocate(size, sizeof(x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Request), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Request__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Request__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Request__Sequence__fini(x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Request__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Request__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Request__Sequence *
x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Request__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Request__Sequence * array = (x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Request__Sequence *)allocator.allocate(sizeof(x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Request__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Request__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Request__Sequence__destroy(x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Request__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Request__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Request__Sequence__are_equal(const x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Request__Sequence * lhs, const x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Request__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Request__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Request__Sequence__copy(
  const x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Request__Sequence * input,
  x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Request__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Request);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Request * data =
      (x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Request *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Request__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Request__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Request__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `stamp`
#include "builtin_interfaces/msg/detail/time__functions.h"

bool
x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Response__init(x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Response * msg)
{
  if (!msg) {
    return false;
  }
  // accepted
  // stamp
  if (!builtin_interfaces__msg__Time__init(&msg->stamp)) {
    x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Response__fini(msg);
    return false;
  }
  return true;
}

void
x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Response__fini(x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Response * msg)
{
  if (!msg) {
    return;
  }
  // accepted
  // stamp
  builtin_interfaces__msg__Time__fini(&msg->stamp);
}

bool
x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Response__are_equal(const x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Response * lhs, const x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Response * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // accepted
  if (lhs->accepted != rhs->accepted) {
    return false;
  }
  // stamp
  if (!builtin_interfaces__msg__Time__are_equal(
      &(lhs->stamp), &(rhs->stamp)))
  {
    return false;
  }
  return true;
}

bool
x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Response__copy(
  const x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Response * input,
  x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Response * output)
{
  if (!input || !output) {
    return false;
  }
  // accepted
  output->accepted = input->accepted;
  // stamp
  if (!builtin_interfaces__msg__Time__copy(
      &(input->stamp), &(output->stamp)))
  {
    return false;
  }
  return true;
}

x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Response *
x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Response__create(void)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Response * msg = (x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Response *)allocator.allocate(sizeof(x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Response), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Response));
  bool success = x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Response__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Response__destroy(x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Response * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Response__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Response__Sequence__init(x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Response__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Response * data = NULL;

  if (size) {
    data = (x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Response *)allocator.zero_allocate(size, sizeof(x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Response), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Response__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Response__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Response__Sequence__fini(x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Response__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Response__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Response__Sequence *
x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Response__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Response__Sequence * array = (x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Response__Sequence *)allocator.allocate(sizeof(x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Response__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Response__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Response__Sequence__destroy(x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Response__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Response__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Response__Sequence__are_equal(const x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Response__Sequence * lhs, const x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Response__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Response__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Response__Sequence__copy(
  const x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Response__Sequence * input,
  x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Response__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Response);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Response * data =
      (x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Response *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Response__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Response__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Response__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `info`
#include "service_msgs/msg/detail/service_event_info__functions.h"
// Member `request`
// Member `response`
// already included above
// #include "x2robot_msgs/action/detail/whole_body_test_once__functions.h"

bool
x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Event__init(x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Event * msg)
{
  if (!msg) {
    return false;
  }
  // info
  if (!service_msgs__msg__ServiceEventInfo__init(&msg->info)) {
    x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Event__fini(msg);
    return false;
  }
  // request
  if (!x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Request__Sequence__init(&msg->request, 0)) {
    x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Event__fini(msg);
    return false;
  }
  // response
  if (!x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Response__Sequence__init(&msg->response, 0)) {
    x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Event__fini(msg);
    return false;
  }
  return true;
}

void
x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Event__fini(x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Event * msg)
{
  if (!msg) {
    return;
  }
  // info
  service_msgs__msg__ServiceEventInfo__fini(&msg->info);
  // request
  x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Request__Sequence__fini(&msg->request);
  // response
  x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Response__Sequence__fini(&msg->response);
}

bool
x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Event__are_equal(const x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Event * lhs, const x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Event * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // info
  if (!service_msgs__msg__ServiceEventInfo__are_equal(
      &(lhs->info), &(rhs->info)))
  {
    return false;
  }
  // request
  if (!x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Request__Sequence__are_equal(
      &(lhs->request), &(rhs->request)))
  {
    return false;
  }
  // response
  if (!x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Response__Sequence__are_equal(
      &(lhs->response), &(rhs->response)))
  {
    return false;
  }
  return true;
}

bool
x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Event__copy(
  const x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Event * input,
  x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Event * output)
{
  if (!input || !output) {
    return false;
  }
  // info
  if (!service_msgs__msg__ServiceEventInfo__copy(
      &(input->info), &(output->info)))
  {
    return false;
  }
  // request
  if (!x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Request__Sequence__copy(
      &(input->request), &(output->request)))
  {
    return false;
  }
  // response
  if (!x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Response__Sequence__copy(
      &(input->response), &(output->response)))
  {
    return false;
  }
  return true;
}

x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Event *
x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Event__create(void)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Event * msg = (x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Event *)allocator.allocate(sizeof(x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Event), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Event));
  bool success = x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Event__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Event__destroy(x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Event * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Event__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Event__Sequence__init(x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Event__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Event * data = NULL;

  if (size) {
    data = (x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Event *)allocator.zero_allocate(size, sizeof(x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Event), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Event__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Event__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Event__Sequence__fini(x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Event__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Event__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Event__Sequence *
x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Event__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Event__Sequence * array = (x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Event__Sequence *)allocator.allocate(sizeof(x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Event__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Event__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Event__Sequence__destroy(x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Event__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Event__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Event__Sequence__are_equal(const x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Event__Sequence * lhs, const x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Event__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Event__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Event__Sequence__copy(
  const x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Event__Sequence * input,
  x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Event__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Event);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Event * data =
      (x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Event *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Event__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Event__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!x2robot_msgs__action__WholeBodyTestOnce_SendGoal_Event__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `goal_id`
// already included above
// #include "unique_identifier_msgs/msg/detail/uuid__functions.h"

bool
x2robot_msgs__action__WholeBodyTestOnce_GetResult_Request__init(x2robot_msgs__action__WholeBodyTestOnce_GetResult_Request * msg)
{
  if (!msg) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__init(&msg->goal_id)) {
    x2robot_msgs__action__WholeBodyTestOnce_GetResult_Request__fini(msg);
    return false;
  }
  return true;
}

void
x2robot_msgs__action__WholeBodyTestOnce_GetResult_Request__fini(x2robot_msgs__action__WholeBodyTestOnce_GetResult_Request * msg)
{
  if (!msg) {
    return;
  }
  // goal_id
  unique_identifier_msgs__msg__UUID__fini(&msg->goal_id);
}

bool
x2robot_msgs__action__WholeBodyTestOnce_GetResult_Request__are_equal(const x2robot_msgs__action__WholeBodyTestOnce_GetResult_Request * lhs, const x2robot_msgs__action__WholeBodyTestOnce_GetResult_Request * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__are_equal(
      &(lhs->goal_id), &(rhs->goal_id)))
  {
    return false;
  }
  return true;
}

bool
x2robot_msgs__action__WholeBodyTestOnce_GetResult_Request__copy(
  const x2robot_msgs__action__WholeBodyTestOnce_GetResult_Request * input,
  x2robot_msgs__action__WholeBodyTestOnce_GetResult_Request * output)
{
  if (!input || !output) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__copy(
      &(input->goal_id), &(output->goal_id)))
  {
    return false;
  }
  return true;
}

x2robot_msgs__action__WholeBodyTestOnce_GetResult_Request *
x2robot_msgs__action__WholeBodyTestOnce_GetResult_Request__create(void)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  x2robot_msgs__action__WholeBodyTestOnce_GetResult_Request * msg = (x2robot_msgs__action__WholeBodyTestOnce_GetResult_Request *)allocator.allocate(sizeof(x2robot_msgs__action__WholeBodyTestOnce_GetResult_Request), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(x2robot_msgs__action__WholeBodyTestOnce_GetResult_Request));
  bool success = x2robot_msgs__action__WholeBodyTestOnce_GetResult_Request__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
x2robot_msgs__action__WholeBodyTestOnce_GetResult_Request__destroy(x2robot_msgs__action__WholeBodyTestOnce_GetResult_Request * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    x2robot_msgs__action__WholeBodyTestOnce_GetResult_Request__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
x2robot_msgs__action__WholeBodyTestOnce_GetResult_Request__Sequence__init(x2robot_msgs__action__WholeBodyTestOnce_GetResult_Request__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  x2robot_msgs__action__WholeBodyTestOnce_GetResult_Request * data = NULL;

  if (size) {
    data = (x2robot_msgs__action__WholeBodyTestOnce_GetResult_Request *)allocator.zero_allocate(size, sizeof(x2robot_msgs__action__WholeBodyTestOnce_GetResult_Request), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = x2robot_msgs__action__WholeBodyTestOnce_GetResult_Request__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        x2robot_msgs__action__WholeBodyTestOnce_GetResult_Request__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
x2robot_msgs__action__WholeBodyTestOnce_GetResult_Request__Sequence__fini(x2robot_msgs__action__WholeBodyTestOnce_GetResult_Request__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      x2robot_msgs__action__WholeBodyTestOnce_GetResult_Request__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

x2robot_msgs__action__WholeBodyTestOnce_GetResult_Request__Sequence *
x2robot_msgs__action__WholeBodyTestOnce_GetResult_Request__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  x2robot_msgs__action__WholeBodyTestOnce_GetResult_Request__Sequence * array = (x2robot_msgs__action__WholeBodyTestOnce_GetResult_Request__Sequence *)allocator.allocate(sizeof(x2robot_msgs__action__WholeBodyTestOnce_GetResult_Request__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = x2robot_msgs__action__WholeBodyTestOnce_GetResult_Request__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
x2robot_msgs__action__WholeBodyTestOnce_GetResult_Request__Sequence__destroy(x2robot_msgs__action__WholeBodyTestOnce_GetResult_Request__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    x2robot_msgs__action__WholeBodyTestOnce_GetResult_Request__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
x2robot_msgs__action__WholeBodyTestOnce_GetResult_Request__Sequence__are_equal(const x2robot_msgs__action__WholeBodyTestOnce_GetResult_Request__Sequence * lhs, const x2robot_msgs__action__WholeBodyTestOnce_GetResult_Request__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!x2robot_msgs__action__WholeBodyTestOnce_GetResult_Request__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
x2robot_msgs__action__WholeBodyTestOnce_GetResult_Request__Sequence__copy(
  const x2robot_msgs__action__WholeBodyTestOnce_GetResult_Request__Sequence * input,
  x2robot_msgs__action__WholeBodyTestOnce_GetResult_Request__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(x2robot_msgs__action__WholeBodyTestOnce_GetResult_Request);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    x2robot_msgs__action__WholeBodyTestOnce_GetResult_Request * data =
      (x2robot_msgs__action__WholeBodyTestOnce_GetResult_Request *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!x2robot_msgs__action__WholeBodyTestOnce_GetResult_Request__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          x2robot_msgs__action__WholeBodyTestOnce_GetResult_Request__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!x2robot_msgs__action__WholeBodyTestOnce_GetResult_Request__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `result`
// already included above
// #include "x2robot_msgs/action/detail/whole_body_test_once__functions.h"

bool
x2robot_msgs__action__WholeBodyTestOnce_GetResult_Response__init(x2robot_msgs__action__WholeBodyTestOnce_GetResult_Response * msg)
{
  if (!msg) {
    return false;
  }
  // status
  // result
  if (!x2robot_msgs__action__WholeBodyTestOnce_Result__init(&msg->result)) {
    x2robot_msgs__action__WholeBodyTestOnce_GetResult_Response__fini(msg);
    return false;
  }
  return true;
}

void
x2robot_msgs__action__WholeBodyTestOnce_GetResult_Response__fini(x2robot_msgs__action__WholeBodyTestOnce_GetResult_Response * msg)
{
  if (!msg) {
    return;
  }
  // status
  // result
  x2robot_msgs__action__WholeBodyTestOnce_Result__fini(&msg->result);
}

bool
x2robot_msgs__action__WholeBodyTestOnce_GetResult_Response__are_equal(const x2robot_msgs__action__WholeBodyTestOnce_GetResult_Response * lhs, const x2robot_msgs__action__WholeBodyTestOnce_GetResult_Response * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // status
  if (lhs->status != rhs->status) {
    return false;
  }
  // result
  if (!x2robot_msgs__action__WholeBodyTestOnce_Result__are_equal(
      &(lhs->result), &(rhs->result)))
  {
    return false;
  }
  return true;
}

bool
x2robot_msgs__action__WholeBodyTestOnce_GetResult_Response__copy(
  const x2robot_msgs__action__WholeBodyTestOnce_GetResult_Response * input,
  x2robot_msgs__action__WholeBodyTestOnce_GetResult_Response * output)
{
  if (!input || !output) {
    return false;
  }
  // status
  output->status = input->status;
  // result
  if (!x2robot_msgs__action__WholeBodyTestOnce_Result__copy(
      &(input->result), &(output->result)))
  {
    return false;
  }
  return true;
}

x2robot_msgs__action__WholeBodyTestOnce_GetResult_Response *
x2robot_msgs__action__WholeBodyTestOnce_GetResult_Response__create(void)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  x2robot_msgs__action__WholeBodyTestOnce_GetResult_Response * msg = (x2robot_msgs__action__WholeBodyTestOnce_GetResult_Response *)allocator.allocate(sizeof(x2robot_msgs__action__WholeBodyTestOnce_GetResult_Response), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(x2robot_msgs__action__WholeBodyTestOnce_GetResult_Response));
  bool success = x2robot_msgs__action__WholeBodyTestOnce_GetResult_Response__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
x2robot_msgs__action__WholeBodyTestOnce_GetResult_Response__destroy(x2robot_msgs__action__WholeBodyTestOnce_GetResult_Response * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    x2robot_msgs__action__WholeBodyTestOnce_GetResult_Response__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
x2robot_msgs__action__WholeBodyTestOnce_GetResult_Response__Sequence__init(x2robot_msgs__action__WholeBodyTestOnce_GetResult_Response__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  x2robot_msgs__action__WholeBodyTestOnce_GetResult_Response * data = NULL;

  if (size) {
    data = (x2robot_msgs__action__WholeBodyTestOnce_GetResult_Response *)allocator.zero_allocate(size, sizeof(x2robot_msgs__action__WholeBodyTestOnce_GetResult_Response), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = x2robot_msgs__action__WholeBodyTestOnce_GetResult_Response__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        x2robot_msgs__action__WholeBodyTestOnce_GetResult_Response__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
x2robot_msgs__action__WholeBodyTestOnce_GetResult_Response__Sequence__fini(x2robot_msgs__action__WholeBodyTestOnce_GetResult_Response__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      x2robot_msgs__action__WholeBodyTestOnce_GetResult_Response__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

x2robot_msgs__action__WholeBodyTestOnce_GetResult_Response__Sequence *
x2robot_msgs__action__WholeBodyTestOnce_GetResult_Response__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  x2robot_msgs__action__WholeBodyTestOnce_GetResult_Response__Sequence * array = (x2robot_msgs__action__WholeBodyTestOnce_GetResult_Response__Sequence *)allocator.allocate(sizeof(x2robot_msgs__action__WholeBodyTestOnce_GetResult_Response__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = x2robot_msgs__action__WholeBodyTestOnce_GetResult_Response__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
x2robot_msgs__action__WholeBodyTestOnce_GetResult_Response__Sequence__destroy(x2robot_msgs__action__WholeBodyTestOnce_GetResult_Response__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    x2robot_msgs__action__WholeBodyTestOnce_GetResult_Response__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
x2robot_msgs__action__WholeBodyTestOnce_GetResult_Response__Sequence__are_equal(const x2robot_msgs__action__WholeBodyTestOnce_GetResult_Response__Sequence * lhs, const x2robot_msgs__action__WholeBodyTestOnce_GetResult_Response__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!x2robot_msgs__action__WholeBodyTestOnce_GetResult_Response__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
x2robot_msgs__action__WholeBodyTestOnce_GetResult_Response__Sequence__copy(
  const x2robot_msgs__action__WholeBodyTestOnce_GetResult_Response__Sequence * input,
  x2robot_msgs__action__WholeBodyTestOnce_GetResult_Response__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(x2robot_msgs__action__WholeBodyTestOnce_GetResult_Response);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    x2robot_msgs__action__WholeBodyTestOnce_GetResult_Response * data =
      (x2robot_msgs__action__WholeBodyTestOnce_GetResult_Response *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!x2robot_msgs__action__WholeBodyTestOnce_GetResult_Response__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          x2robot_msgs__action__WholeBodyTestOnce_GetResult_Response__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!x2robot_msgs__action__WholeBodyTestOnce_GetResult_Response__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `info`
// already included above
// #include "service_msgs/msg/detail/service_event_info__functions.h"
// Member `request`
// Member `response`
// already included above
// #include "x2robot_msgs/action/detail/whole_body_test_once__functions.h"

bool
x2robot_msgs__action__WholeBodyTestOnce_GetResult_Event__init(x2robot_msgs__action__WholeBodyTestOnce_GetResult_Event * msg)
{
  if (!msg) {
    return false;
  }
  // info
  if (!service_msgs__msg__ServiceEventInfo__init(&msg->info)) {
    x2robot_msgs__action__WholeBodyTestOnce_GetResult_Event__fini(msg);
    return false;
  }
  // request
  if (!x2robot_msgs__action__WholeBodyTestOnce_GetResult_Request__Sequence__init(&msg->request, 0)) {
    x2robot_msgs__action__WholeBodyTestOnce_GetResult_Event__fini(msg);
    return false;
  }
  // response
  if (!x2robot_msgs__action__WholeBodyTestOnce_GetResult_Response__Sequence__init(&msg->response, 0)) {
    x2robot_msgs__action__WholeBodyTestOnce_GetResult_Event__fini(msg);
    return false;
  }
  return true;
}

void
x2robot_msgs__action__WholeBodyTestOnce_GetResult_Event__fini(x2robot_msgs__action__WholeBodyTestOnce_GetResult_Event * msg)
{
  if (!msg) {
    return;
  }
  // info
  service_msgs__msg__ServiceEventInfo__fini(&msg->info);
  // request
  x2robot_msgs__action__WholeBodyTestOnce_GetResult_Request__Sequence__fini(&msg->request);
  // response
  x2robot_msgs__action__WholeBodyTestOnce_GetResult_Response__Sequence__fini(&msg->response);
}

bool
x2robot_msgs__action__WholeBodyTestOnce_GetResult_Event__are_equal(const x2robot_msgs__action__WholeBodyTestOnce_GetResult_Event * lhs, const x2robot_msgs__action__WholeBodyTestOnce_GetResult_Event * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // info
  if (!service_msgs__msg__ServiceEventInfo__are_equal(
      &(lhs->info), &(rhs->info)))
  {
    return false;
  }
  // request
  if (!x2robot_msgs__action__WholeBodyTestOnce_GetResult_Request__Sequence__are_equal(
      &(lhs->request), &(rhs->request)))
  {
    return false;
  }
  // response
  if (!x2robot_msgs__action__WholeBodyTestOnce_GetResult_Response__Sequence__are_equal(
      &(lhs->response), &(rhs->response)))
  {
    return false;
  }
  return true;
}

bool
x2robot_msgs__action__WholeBodyTestOnce_GetResult_Event__copy(
  const x2robot_msgs__action__WholeBodyTestOnce_GetResult_Event * input,
  x2robot_msgs__action__WholeBodyTestOnce_GetResult_Event * output)
{
  if (!input || !output) {
    return false;
  }
  // info
  if (!service_msgs__msg__ServiceEventInfo__copy(
      &(input->info), &(output->info)))
  {
    return false;
  }
  // request
  if (!x2robot_msgs__action__WholeBodyTestOnce_GetResult_Request__Sequence__copy(
      &(input->request), &(output->request)))
  {
    return false;
  }
  // response
  if (!x2robot_msgs__action__WholeBodyTestOnce_GetResult_Response__Sequence__copy(
      &(input->response), &(output->response)))
  {
    return false;
  }
  return true;
}

x2robot_msgs__action__WholeBodyTestOnce_GetResult_Event *
x2robot_msgs__action__WholeBodyTestOnce_GetResult_Event__create(void)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  x2robot_msgs__action__WholeBodyTestOnce_GetResult_Event * msg = (x2robot_msgs__action__WholeBodyTestOnce_GetResult_Event *)allocator.allocate(sizeof(x2robot_msgs__action__WholeBodyTestOnce_GetResult_Event), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(x2robot_msgs__action__WholeBodyTestOnce_GetResult_Event));
  bool success = x2robot_msgs__action__WholeBodyTestOnce_GetResult_Event__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
x2robot_msgs__action__WholeBodyTestOnce_GetResult_Event__destroy(x2robot_msgs__action__WholeBodyTestOnce_GetResult_Event * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    x2robot_msgs__action__WholeBodyTestOnce_GetResult_Event__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
x2robot_msgs__action__WholeBodyTestOnce_GetResult_Event__Sequence__init(x2robot_msgs__action__WholeBodyTestOnce_GetResult_Event__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  x2robot_msgs__action__WholeBodyTestOnce_GetResult_Event * data = NULL;

  if (size) {
    data = (x2robot_msgs__action__WholeBodyTestOnce_GetResult_Event *)allocator.zero_allocate(size, sizeof(x2robot_msgs__action__WholeBodyTestOnce_GetResult_Event), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = x2robot_msgs__action__WholeBodyTestOnce_GetResult_Event__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        x2robot_msgs__action__WholeBodyTestOnce_GetResult_Event__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
x2robot_msgs__action__WholeBodyTestOnce_GetResult_Event__Sequence__fini(x2robot_msgs__action__WholeBodyTestOnce_GetResult_Event__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      x2robot_msgs__action__WholeBodyTestOnce_GetResult_Event__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

x2robot_msgs__action__WholeBodyTestOnce_GetResult_Event__Sequence *
x2robot_msgs__action__WholeBodyTestOnce_GetResult_Event__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  x2robot_msgs__action__WholeBodyTestOnce_GetResult_Event__Sequence * array = (x2robot_msgs__action__WholeBodyTestOnce_GetResult_Event__Sequence *)allocator.allocate(sizeof(x2robot_msgs__action__WholeBodyTestOnce_GetResult_Event__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = x2robot_msgs__action__WholeBodyTestOnce_GetResult_Event__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
x2robot_msgs__action__WholeBodyTestOnce_GetResult_Event__Sequence__destroy(x2robot_msgs__action__WholeBodyTestOnce_GetResult_Event__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    x2robot_msgs__action__WholeBodyTestOnce_GetResult_Event__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
x2robot_msgs__action__WholeBodyTestOnce_GetResult_Event__Sequence__are_equal(const x2robot_msgs__action__WholeBodyTestOnce_GetResult_Event__Sequence * lhs, const x2robot_msgs__action__WholeBodyTestOnce_GetResult_Event__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!x2robot_msgs__action__WholeBodyTestOnce_GetResult_Event__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
x2robot_msgs__action__WholeBodyTestOnce_GetResult_Event__Sequence__copy(
  const x2robot_msgs__action__WholeBodyTestOnce_GetResult_Event__Sequence * input,
  x2robot_msgs__action__WholeBodyTestOnce_GetResult_Event__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(x2robot_msgs__action__WholeBodyTestOnce_GetResult_Event);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    x2robot_msgs__action__WholeBodyTestOnce_GetResult_Event * data =
      (x2robot_msgs__action__WholeBodyTestOnce_GetResult_Event *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!x2robot_msgs__action__WholeBodyTestOnce_GetResult_Event__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          x2robot_msgs__action__WholeBodyTestOnce_GetResult_Event__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!x2robot_msgs__action__WholeBodyTestOnce_GetResult_Event__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `goal_id`
// already included above
// #include "unique_identifier_msgs/msg/detail/uuid__functions.h"
// Member `feedback`
// already included above
// #include "x2robot_msgs/action/detail/whole_body_test_once__functions.h"

bool
x2robot_msgs__action__WholeBodyTestOnce_FeedbackMessage__init(x2robot_msgs__action__WholeBodyTestOnce_FeedbackMessage * msg)
{
  if (!msg) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__init(&msg->goal_id)) {
    x2robot_msgs__action__WholeBodyTestOnce_FeedbackMessage__fini(msg);
    return false;
  }
  // feedback
  if (!x2robot_msgs__action__WholeBodyTestOnce_Feedback__init(&msg->feedback)) {
    x2robot_msgs__action__WholeBodyTestOnce_FeedbackMessage__fini(msg);
    return false;
  }
  return true;
}

void
x2robot_msgs__action__WholeBodyTestOnce_FeedbackMessage__fini(x2robot_msgs__action__WholeBodyTestOnce_FeedbackMessage * msg)
{
  if (!msg) {
    return;
  }
  // goal_id
  unique_identifier_msgs__msg__UUID__fini(&msg->goal_id);
  // feedback
  x2robot_msgs__action__WholeBodyTestOnce_Feedback__fini(&msg->feedback);
}

bool
x2robot_msgs__action__WholeBodyTestOnce_FeedbackMessage__are_equal(const x2robot_msgs__action__WholeBodyTestOnce_FeedbackMessage * lhs, const x2robot_msgs__action__WholeBodyTestOnce_FeedbackMessage * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__are_equal(
      &(lhs->goal_id), &(rhs->goal_id)))
  {
    return false;
  }
  // feedback
  if (!x2robot_msgs__action__WholeBodyTestOnce_Feedback__are_equal(
      &(lhs->feedback), &(rhs->feedback)))
  {
    return false;
  }
  return true;
}

bool
x2robot_msgs__action__WholeBodyTestOnce_FeedbackMessage__copy(
  const x2robot_msgs__action__WholeBodyTestOnce_FeedbackMessage * input,
  x2robot_msgs__action__WholeBodyTestOnce_FeedbackMessage * output)
{
  if (!input || !output) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__copy(
      &(input->goal_id), &(output->goal_id)))
  {
    return false;
  }
  // feedback
  if (!x2robot_msgs__action__WholeBodyTestOnce_Feedback__copy(
      &(input->feedback), &(output->feedback)))
  {
    return false;
  }
  return true;
}

x2robot_msgs__action__WholeBodyTestOnce_FeedbackMessage *
x2robot_msgs__action__WholeBodyTestOnce_FeedbackMessage__create(void)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  x2robot_msgs__action__WholeBodyTestOnce_FeedbackMessage * msg = (x2robot_msgs__action__WholeBodyTestOnce_FeedbackMessage *)allocator.allocate(sizeof(x2robot_msgs__action__WholeBodyTestOnce_FeedbackMessage), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(x2robot_msgs__action__WholeBodyTestOnce_FeedbackMessage));
  bool success = x2robot_msgs__action__WholeBodyTestOnce_FeedbackMessage__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
x2robot_msgs__action__WholeBodyTestOnce_FeedbackMessage__destroy(x2robot_msgs__action__WholeBodyTestOnce_FeedbackMessage * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    x2robot_msgs__action__WholeBodyTestOnce_FeedbackMessage__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
x2robot_msgs__action__WholeBodyTestOnce_FeedbackMessage__Sequence__init(x2robot_msgs__action__WholeBodyTestOnce_FeedbackMessage__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  x2robot_msgs__action__WholeBodyTestOnce_FeedbackMessage * data = NULL;

  if (size) {
    data = (x2robot_msgs__action__WholeBodyTestOnce_FeedbackMessage *)allocator.zero_allocate(size, sizeof(x2robot_msgs__action__WholeBodyTestOnce_FeedbackMessage), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = x2robot_msgs__action__WholeBodyTestOnce_FeedbackMessage__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        x2robot_msgs__action__WholeBodyTestOnce_FeedbackMessage__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
x2robot_msgs__action__WholeBodyTestOnce_FeedbackMessage__Sequence__fini(x2robot_msgs__action__WholeBodyTestOnce_FeedbackMessage__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      x2robot_msgs__action__WholeBodyTestOnce_FeedbackMessage__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

x2robot_msgs__action__WholeBodyTestOnce_FeedbackMessage__Sequence *
x2robot_msgs__action__WholeBodyTestOnce_FeedbackMessage__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  x2robot_msgs__action__WholeBodyTestOnce_FeedbackMessage__Sequence * array = (x2robot_msgs__action__WholeBodyTestOnce_FeedbackMessage__Sequence *)allocator.allocate(sizeof(x2robot_msgs__action__WholeBodyTestOnce_FeedbackMessage__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = x2robot_msgs__action__WholeBodyTestOnce_FeedbackMessage__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
x2robot_msgs__action__WholeBodyTestOnce_FeedbackMessage__Sequence__destroy(x2robot_msgs__action__WholeBodyTestOnce_FeedbackMessage__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    x2robot_msgs__action__WholeBodyTestOnce_FeedbackMessage__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
x2robot_msgs__action__WholeBodyTestOnce_FeedbackMessage__Sequence__are_equal(const x2robot_msgs__action__WholeBodyTestOnce_FeedbackMessage__Sequence * lhs, const x2robot_msgs__action__WholeBodyTestOnce_FeedbackMessage__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!x2robot_msgs__action__WholeBodyTestOnce_FeedbackMessage__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
x2robot_msgs__action__WholeBodyTestOnce_FeedbackMessage__Sequence__copy(
  const x2robot_msgs__action__WholeBodyTestOnce_FeedbackMessage__Sequence * input,
  x2robot_msgs__action__WholeBodyTestOnce_FeedbackMessage__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(x2robot_msgs__action__WholeBodyTestOnce_FeedbackMessage);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    x2robot_msgs__action__WholeBodyTestOnce_FeedbackMessage * data =
      (x2robot_msgs__action__WholeBodyTestOnce_FeedbackMessage *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!x2robot_msgs__action__WholeBodyTestOnce_FeedbackMessage__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          x2robot_msgs__action__WholeBodyTestOnce_FeedbackMessage__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!x2robot_msgs__action__WholeBodyTestOnce_FeedbackMessage__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}
