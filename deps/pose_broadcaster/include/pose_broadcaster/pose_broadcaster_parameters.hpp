...
[[nodiscard]] auto to_parameter_value(rsl::StaticVector<T, capacity> const& value) {
  ...
  public:
    ...
    void declare_params(){
      ...
      RCLCPP_DEBUG_STREAM(logger_, (prefix_ + "pose_name") << ": " << param.get_type_name() << " = " << param.value_to_string());
      if(auto validation_result = not_empty<std::string>(param);
        !validation_result) {
      ...
      RCLCPP_DEBUG_STREAM(logger_, (prefix_ + "tf.enable") << ": " << param.get_type_name() << " = " << param.value_to_string());
      updated_params.tf.enable = param.as_bool();
      param = parameters_interface_->get_parameter(prefix_ + "tf.child_frame_id");
      RCLCPP_DEBUG_STREAM(logger_, (prefix_ + "tf.child_frame_id") << ": " << param.get_type_name() << " = " << param.value_to_string());
      updated_params.tf.child_frame_id = param.as_string();
