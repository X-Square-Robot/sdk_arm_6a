...
[[nodiscard]] auto to_parameter_value(rsl::StaticVector<T, capacity> const& value) {
  ...
  public:
    ...
    void declare_params(){
      ...
      RCLCPP_DEBUG_STREAM(logger_, (prefix_ + "traction_wheel_radius") << ": " << param.get_type_name() << " = " << param.value_to_string());
      if(auto validation_result = gt<double>(param, 0.0);
        !validation_result) {
          throw rclcpp::exceptions::InvalidParameterValueException(fmt::format("Invalid value set during initialization for parameter 'traction_wheel_radius': {}", validation_result.error()));
      }
      updated_params.traction_wheel_radius = param.as_double();
